...
public class GameCreationScreen {
    ...
    public static void print(Terminal terminal) {
        ...
        terminal.writer().print(printGameCreationScreen());
        terminal.flush();
    }
     ...
     */
    private static Ansi printGameCreationScreen() {
        Ansi ansi = new Ansi();
        printTitle(ansi);
        moveCursor(ansi, it.polimi.ingsw.view.cli.coordinates.Subtitle.getInstance());
        printSubtitle(ansi);
        moveCursor(ansi, GameCreationSettings.getInstance());
        printSettings(ansi);
        return ansi;
    }
    ...
    static void printSettings(Ansi ansi) {
        ...
        ansi.append("┌─────────────────────┐");
        newLine(ansi, false);
        ansi.append("  Players number:      ");
        newLine(ansi, false);
        ansi.append("  Difficulty:          ");
        newLine(ansi, false);
        ansi.append("└─────────────────────┘");
        moveCursor(ansi, GameCreationSettingsInput.getInstance());
    ...
    private static void newLine(Ansi ansi, boolean title) {
        if (title)
            ...
            moveCursor(ansi, GameCreationNewLine.getInstance());
    }
