...
public class Cloud {
    ...
    public boolean isFull() {
        if (this.students.get(HouseColor.BLUE) != 0) return true;
        if (this.students.get(HouseColor.FUCHSIA) != 0) return true;
        if (this.students.get(HouseColor.GREEN) != 0) return true;
        if (this.students.get(HouseColor.YELLOW) != 0) return true;
        if (this.students.get(HouseColor.RED) != 0) return true;
        return false;
    }
    ...
    public void refill(Map<HouseColor, Integer> newStudents) {
        Arrays.stream(HouseColor.values()).forEach(color -> {
            this.students.put(color, newStudents.get(color));
        });
    }
